...
class DomainToolsConnector:
    ...
    def _enrich_domaintools(self, builder: DtBuilder, observable: dict) -> str:
        ...
        if observable["entity_type"] == "Domain-Name":
            ...
            return f"Entity type of the observable: {observable['entity_type']} not supported."
        ...
        for entry in results:
            ...
            for ip in entry.get("ip", ()):
                if "address" in ip:
                    _, ip_id = builder.link_domain_resolves_to(
                        domain_source_id,
                        ip["address"]["value"],
                        EntityType.IPV4,
                        creation_date,
                ...
                for values in entry.get(category, ()):
                    if (domain := values["domain"]["value"]) != entry["domain"]:
                        if not validators.domain(domain):
                            ...
                            continue
                        _, new_domain_id = builder.link_domain_resolves_to(
                            domain_source_id,
                            domain,
